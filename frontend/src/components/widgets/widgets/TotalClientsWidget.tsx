...
export default function TotalClientsWidget({ data }: Props) {
  ...
  const value = Number(data?.totalClients ?? 0);
  return (
    <>
      <div style={{ marginBottom: 8, color: 'var(--brand, var(--primary))' }}>
        <PlatformIcon name="users" size={iconSize} strokeWidth={1.5} />
      </div>
      {value > 0 ? (
      ...
      <div className="small" style={{ color: 'var(--ink-soft, var(--text-muted))' }}>Всего клиентов</div>
    </>
  );
}
