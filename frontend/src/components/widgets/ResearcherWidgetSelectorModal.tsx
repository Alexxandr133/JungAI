...
interface ResearcherWidgetSelectorModalProps {
  onClose: () => void;
  onAdd: (type: ResearcherWidgetType) => void;
  existingWidgets: ResearcherWidgetType[];
}

export default function ResearcherWidgetSelectorModal({
  onClose,
  onAdd,
  existingWidgets
}: ResearcherWidgetSelectorModalProps) {
  const [searchQuery, setSearchQuery] = useState('');

  const availableWidgets = Object.values(RESEARCHER_WIDGET_DEFINITIONS).filter(
    (widget) =>
  ...
  const handleAdd = (type: ResearcherWidgetType) => {
    onAdd(type);
    onClose();
  };
