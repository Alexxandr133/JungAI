...
/** ╨Я╤А╨╕╨▓╨╛╨┤╨╕╤В ╤Б╨╛╤Е╤А╨░╨╜╤С╨╜╨╜╤Л╨╣ JSON (v1 ╨▒╨╡╨╖ kind ╨╕╨╗╨╕ v2) ╨║ TestResult ╨┤╨╗╤П ╤Н╨║╤А╨░╨╜╨░ */
...
export default function ClientTests({ persistResults = true }: ClientTestsProps) {
  ...
  // тАФтАФтАФ ╨а╨╡╨╖╤Г╨╗╤М╤В╨░╤В╤Л: ╨░╤А╤Е╨╡╤В╨╕╨┐ тАФтАФтАФ
        ...
        <main style={{ flex: 1, padding: '24px clamp(16px, 5vw, 48px)', maxWidth: '100%', overflowX: 'hidden' }}>
          <div className="card" style={{ padding: 24, maxWidth: 800, margin: '0 auto' }}>
            <div style={{ marginBottom: 20, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <h2 style={{ margin: 0 }}>╨а╨╡╨╖╤Г╨╗╤М╤В╨░╤В: {d.name}</h2>
            ...
            <div className="card" style={{ padding: 20, marginBottom: 16, background: 'var(--surface-2)' }}>
              <div style={{ fontWeight: 600, marginBottom: 8 }}>╨Ю╨┐╨╕╤Б╨░╨╜╨╕╨╡</div>
              <div style={{ lineHeight: 1.7 }}>{d.description}</div>
            </div>
            <div className="card" style={{ padding: 20, marginBottom: 16 }}>
              <div style={{ fontWeight: 600, marginBottom: 8 }}>╨Э╨░ ╨╖╨░╨╝╨╡╤В╨║╤Г</div>
    ...
    return (
      <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
        ...
        <main style={{ flex: 1, padding: '24px clamp(16px, 5vw, 48px)', maxWidth: '100%', overflowX: 'hidden' }}>
          <div className="card" style={{ padding: 24, maxWidth: 800, margin: '0 auto' }}>
            <div style={{ marginBottom: 20, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <h2 style={{ margin: 0 }}>╨в╨╡╨╝╤Л ╨┤╨╗╤П ╤А╨░╨▒╨╛╤В╤Л ╤Б ╨в╨╡╨╜╤М╤О</h2>
              <button className="button secondary" onClick={closeResult} style={{ padding: '6px 12px', fontSize: 13 }}>
                ╨Ч╨░╨║╤А╤Л╤В╤М
              </button>
            </div>
            <p className="small" style={{ color: 'var(--text-muted)', marginBottom: 20 }}>
              ╨Ш╨╜╤Б╤В╤А╤Г╨╝╨╡╨╜╤В ╤Б╨░╨╝╨╛╨╜╨░╨▒╨╗╤О╨┤╨╡╨╜╨╕╤П, ╨╜╨╡ ╨║╨╗╨╕╨╜╨╕╤З╨╡╤Б╨║╨░╤П ╨╛╤Ж╨╡╨╜╨║╨░.
            </p>
            ...
            {testResult.shadowScores && (
              <div style={{ marginTop: 20, display: 'grid', gap: 8 }}>
                <div style={{ fontWeight: 700 }}>╨Э╨░╨▒╤А╨░╨╜╨╛ ╨┐╨╛ ╤В╨╡╨╝╨░╨╝</div>
  ...
  // тАФтАФтАФ ╨Ы╨╕╤З╨╜╨╛╤Б╤В╤М / ╨в╨Р тАФтАФтАФ
    ...
    return (
      <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
        ...
        <main style={{ flex: 1, padding: '24px clamp(16px, 5vw, 48px)', maxWidth: '100%', overflowX: 'hidden' }}>
          <div className="card" style={{ padding: 24, maxWidth: 800, margin: '0 auto' }}>
            <div style={{ marginBottom: 20, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <h2 style={{ margin: 0 }}>╨Ф╨╛╨╝╨╕╨╜╨╕╤А╤Г╤О╤Й╨╕╨╣ ╨┤╤А╨░╨╣╨▓╨╡╤А: {d.name}</h2>
              <button className="button secondary" onClick={closeResult} style={{ padding: '6px 12px', fontSize: 13 }}>
                ╨Ч╨░╨║╤А╤Л╤В╤М
              </button>
            </div>
            ...
            {testResult.driverScores && (
              <div style={{ marginTop: 20, display: 'grid', gap: 8 }}>
                <div style={{ fontWeight: 700 }}>╨С╨░╨╗╨╗╤Л ╨┐╨╛ ╨┤╤А╨░╨╣╨▓╨╡╤А╨░╨╝</div>
  ...
  // ╨Ш-╨ж╨╖╨╕╨╜ ╤А╨╡╨╖╤Г╨╗╤М╤В╨░╤В (╨╕ ╨╖╨░╨│╤А╤Г╨╖╨║╨░ ╤Б╤В╨░╤А╤Л╤Е ╨╖╨░╨┐╨╕╤Б╨╡╨╣ ╨▒╨╡╨╖ kind)
        ...
        <main style={{ flex: 1, padding: '24px clamp(16px, 5vw, 48px)', maxWidth: '100%', overflowX: 'hidden' }}>
          <div className="card" style={{ padding: 24, maxWidth: 1000, margin: '0 auto' }}>
            <div style={{ marginBottom: 20, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <h2 style={{ margin: 0 }}>╨У╨╡╨║╤Б╨░╨│╤А╨░╨╝╨╝╨░ {hex.number}: {hex.name}</h2>
              <button className="button secondary" onClick={closeResult} style={{ padding: '6px 12px', fontSize: 13 }}>
                ╨Ч╨░╨║╤А╤Л╤В╤М
              </button>
            </div>
            {testResult.question && (
              <div className="card" style={{ padding: 16, marginBottom: 24, background: 'var(--surface-2)' }}>
                <div style={{ fontWeight: 600, marginBottom: 8 }}>╨Т╨░╤И ╨▓╨╛╨┐╤А╨╛╤Б</div>
            ...
            <div className="card" style={{ padding: 24, marginBottom: 24, textAlign: 'center' }}>
                ...
                {hex.lines.map((line, idx) => (
                  <div key={idx} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    ...
                    <div style={{ fontSize: 14, color: 'var(--text-muted)', minWidth: 120, textAlign: 'left' }}>
                      {line.value} {line.isYang ? 'тЪК' : 'тЪЛ'} {line.isChanging ? '(╨╝╨╡╨╜╤П╤О╤Й╨░╤П╤Б╤П)' : ''}
                    </div>
                  </div>
                ))}
              </div>
              {hex.lines.some((l) => l.isChanging) && (
                <div style={{ padding: 16, background: 'var(--surface-2)', borderRadius: 12, marginBottom: 16, textAlign: 'left' }}>
                  <div style={{ fontWeight: 600, marginBottom: 8 }}>╨Ь╨╡╨╜╤П╤О╤Й╨╕╨╡╤Б╤П ╨╗╨╕╨╜╨╕╨╕</div>
                  {hex.lines.map(
                    (line, idx) =>
                      line.isChanging && (
                        <div key={idx} style={{ marginBottom: 8 }}>
                          ╨Ы╨╕╨╜╨╕╤П {idx + 1}: {hex.linesText[idx]}
                        </div>
                      )
                  )}
                </div>
            ...
            <div className="card" style={{ padding: 20, marginBottom: 24 }}>
              <div style={{ fontWeight: 700, fontSize: 18, marginBottom: 12 }}>╨б╤Г╨╢╨┤╨╡╨╜╨╕╨╡</div>
              <div style={{ lineHeight: 1.8 }}>{hex.judgment}</div>
            </div>
            <div className="card" style={{ padding: 20, marginBottom: 24 }}>
              <div style={{ fontWeight: 700, fontSize: 18, marginBottom: 12 }}>╨Ю╨▒╤А╨░╨╖</div>
              <div style={{ lineHeight: 1.8 }}>{hex.image}</div>
            </div>
            <div className="card" style={{ padding: 20, marginBottom: 24 }}>
              <div style={{ fontWeight: 700, fontSize: 18, marginBottom: 12 }}>╨в╨╡╨║╤Б╤В╤Л ╨╗╨╕╨╜╨╕╨╣</div>
              <div style={{ display: 'grid', gap: 12 }}>
                {hex.linesText.map((text, idx) => (
                  <div key={idx} style={{ padding: 12, background: 'var(--surface-2)', borderRadius: 8, textAlign: 'left' }}>
                    <div style={{ fontWeight: 600, marginBottom: 4, color: 'var(--primary)' }}>╨Ы╨╕╨╜╨╕╤П {idx + 1}</div>
            ...
            {hex.changingTo != null && (
              ...
              >
                <div style={{ fontWeight: 700, fontSize: 18, marginBottom: 12 }}>╨Я╨╡╤А╨╡╤Е╨╛╨┤ ╨║ ╨│╨╡╨║╤Б╨░╨│╤А╨░╨╝╨╝╨╡ {hex.changingTo}</div>
                <div style={{ lineHeight: 1.8 }}>
                  ╨Х╤Б╤В╤М ╨╝╨╡╨╜╤П╤О╤Й╨╕╨╡╤Б╤П ╨╗╨╕╨╜╨╕╨╕ тАФ ╨╕╨╖╤Г╤З╨╕╤В╨╡ ╤В╨░╨║╨╢╨╡ {hex.changingTo}-╤О ╨│╨╡╨║╤Б╨░╨│╤А╨░╨╝╨╝╤Г ╨║╨░╨║ ╨▓╨╡╨║╤В╨╛╤А ╤Б╨┤╨▓╨╕╨│╨░ ╤Б╨╕╤В╤Г╨░╤Ж╨╕╨╕.
                </div>
              </div>
            )}
          </div>
    ...
    return (
      <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
        ...
        <main style={{ flex: 1, padding: '24px clamp(16px, 5vw, 48px)', maxWidth: '100%', overflowX: 'hidden' }}>
          <div className="card" style={{ padding: 24, maxWidth: 1000, margin: '0 auto' }}>
            <div style={{ marginBottom: 20, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <h2 style={{ margin: 0 }}>╨б╨┐╨╕╤А╨░╨╗╤М╨╜╨░╤П ╨┤╨╕╨╜╨░╨╝╨╕╨║╨░: ╨┐╤А╨╛╤Д╨╕╨╗╤М</h2>
              <button className="button secondary" onClick={closeResult} style={{ padding: '6px 12px', fontSize: 13 }}>
                ╨Ч╨░╨║╤А╤Л╤В╤М
              </button>
            </div>
            ...
            >
              ...
              <div style={{ marginTop: 16, padding: 16, background: 'var(--surface-2)', borderRadius: 10 }}>
                <div style={{ fontWeight: 600, marginBottom: 8 }}>╨Ш╨╜╤В╨╡╤А╨┐╤А╨╡╤В╨░╤Ж╨╕╤П</div>
                <div style={{ lineHeight: 1.6 }}>{testResult.interpretation}</div>
              </div>
            </div>
            <div style={{ marginBottom: 24 }}>
              <div style={{ fontWeight: 700, marginBottom: 16, fontSize: 18 }}>╨Я╨╛ ╤Г╤А╨╛╨▓╨╜╤П╨╝</div>
              <div style={{ display: 'grid', gap: 12 }}>
                {levels.map((level) => {
      ...
      <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
        ...
        <main style={{ flex: 1, padding: '24px clamp(16px, 5vw, 48px)' }}>
          ...
          <div style={{ textAlign: 'center', marginTop: 16 }}>
            <button className="button secondary" onClick={closeResult}>тЖР ╨Ъ ╤Б╨┐╨╕╤Б╨║╤Г ╤В╨╡╤Б╤В╨╛╨▓</button>
          </div>
        </main>
      </div>
    );
      ...
      <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
        ...
        <main style={{ flex: 1, padding: '24px clamp(16px, 5vw, 48px)', maxWidth: 800, margin: '0 auto' }}>
          <div className="card" style={{ padding: 24 }}>
            ...
            <button className="button secondary" onClick={closeResult} style={{ marginTop: 16 }}>тЖР ╨Ъ ╤Б╨┐╨╕╤Б╨║╤Г ╤В╨╡╤Б╤В╨╛╨▓</button>
          </div>
        </main>
      </div>
    );
          ...
          <main style={{ flex: 1, padding: '24px clamp(16px, 5vw, 48px)', maxWidth: '100%', overflowX: 'hidden' }}>
            <div className="card" style={{ padding: 24, maxWidth: 800, margin: '0 auto' }}>
              ...
              <div style={{ textAlign: 'center', marginBottom: 32 }}>
                <div style={{ fontSize: 64, marginBottom: 16 }}>тШпя╕П</div>
                <h2 style={{ margin: 0, marginBottom: 12 }}>╨Ш-╨ж╨╖╨╕╨╜: ╨▒╤А╨╛╤Б╨░╨╜╨╕╨╡ ╨╝╨╛╨╜╨╡╤В</h2>
                <div className="small" style={{ color: 'var(--text-muted)' }}>
                  ╨и╨╡╤Б╤В╤М ╨╗╨╕╨╜╨╕╨╣ ╤Б╤В╤А╨╛╤П╤В ╨│╨╡╨║╤Б╨░╨│╤А╨░╨╝╨╝╤Г ╨┐╨╛ ╨┐╨╛╤Б╨╗╨╡╨┤╨╛╨▓╨░╤В╨╡╨╗╤М╨╜╨╛╤Б╤В╨╕ ╨Ъ╨╕╨╜╨│ ╨Т╤Н╨╜╨░ (1тАУ64).
                </div>
              </div>
              <label style={{ display: 'block', marginBottom: 8, fontWeight: 600 }}>╨б╤Д╨╛╤А╨╝╤Г╨╗╨╕╤А╤Г╨╣╤В╨╡ ╨▓╨╛╨┐╤А╨╛╤Б</label>
              <textarea
                value={iChingQuestion}
                onChange={(e) => setIChingQuestion(e.target.value)}
                placeholder="╨Э╨░╨┐╤А╨╕╨╝╨╡╤А: ╤З╤В╨╛ ╨▓╨░╨╢╨╜╨╛ ╤Г╤З╨╡╤Б╤В╤М ╨▓ ╨╝╨╛╨╡╨╣ ╤В╨╡╨║╤Г╤Й╨╡╨╣ ╤Б╨╕╤В╤Г╨░╤Ж╨╕╨╕?"
                style={{
                  width: '100%',
                  padding: '16px',
                  borderRadius: 12,
        ...
        <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
          ...
          >
            <div className="card" style={{ padding: 32, maxWidth: 600, width: '100%', textAlign: 'center' }}>
              <h3 style={{ margin: '0 0 8px' }}>╨Ы╨╕╨╜╨╕╤П {Math.min(iChingLines.length + (isCasting ? 0 : 1), 6)} ╨╕╨╖ 6</h3>
              <div style={{ margin: '24px 0', minHeight: 120, display: 'grid', placeItems: 'center' }}>
                {isCasting ? (
                  <div style={{ fontSize: 40 }}>ЁЯкЩ</div>
                ) : (
                  <div style={{ fontSize: 40, opacity: 0.5 }}>ЁЯкЩ</div>
                )}
              </div>
              {iChingLines.length > 0 && (
                <div style={{ marginBottom: 24, textAlign: 'left' }}>
                  <div style={{ fontWeight: 600, marginBottom: 8 }}>╨б╨╜╨╕╨╖╤Г ╨▓╨▓╨╡╤А╤Е</div>
          ...
          <div className="card" style={{ padding: 24, maxWidth: 800, margin: '0 auto' }}>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <div className="small" style={{ marginTop: 8, color: 'var(--text-muted)' }}>
                ╨Т╨╛╨┐╤А╨╛╤Б {currentQuestion + 1} ╨╕╨╖ {totalQuestions}
              </div>
            </div>
            <div style={{ marginBottom: 20 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
              ...
              <button
                ...
                style={{ padding: '10px 20px' }}
              >
                {currentQuestion === totalQuestions - 1 ? '╨Ч╨░╨▓╨╡╤А╤И╨╕╤В╤М' : '╨Ф╨░╨╗╨╡╨╡ тЖТ'}
              </button>
            </div>
          </div>
        </main>
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      <main style={{ flex: 1, padding: '24px clamp(16px, 5vw, 48px)', maxWidth: '100%', overflowX: 'hidden' }}>
        <div style={{ marginBottom: 24 }}>
          <h1 style={{ margin: 0, fontSize: 32, fontWeight: 800, marginBottom: 8 }}>╨в╨╡╤Б╤В╤Л ╨╕ ╤В╨╡╤Е╨╜╨╕╨║╨╕</h1>
          <p className="small" style={{ color: 'var(--text-muted)', margin: 0 }}>
            ╨б╨║╤А╨╕╨╜╨╕╨╜╨│╨╕ ╨╕ ╤Б╨░╨╝╨╛╨╜╨░╨▒╨╗╤О╨┤╨╡╨╜╨╕╨╡; ╨╜╨╡ ╨╖╨░╨╝╨╡╨╜╤П╤О╤В ╨║╨╛╨╜╤Б╤Г╨╗╤М╤В╨░╤Ж╨╕╤О ╤Б╨┐╨╡╤Ж╨╕╨░╨╗╨╕╤Б╤В╨░.
            {canPersist && loadingSaved && ' ╨Ч╨░╨│╤А╤Г╨╖╨║╨░ ╤Б╨╛╤Е╤А╨░╨╜╤С╨╜╨╜╤Л╤Е ╤А╨╡╨╖╤Г╨╗╤М╤В╨░╤В╨╛╨▓тАж'}
          </p>
          {!persistResults && (
            <p className="small" style={{ color: 'var(--accent)', marginTop: 8 }}>
              ╨а╨╡╨╢╨╕╨╝ ╨│╨╛╤Б╤В╤П: ╤А╨╡╨╖╤Г╨╗╤М╤В╨░╤В╤Л ╨╜╨╡ ╤Б╨╛╤Е╤А╨░╨╜╤П╤О╤В╤Б╤П.
            </p>
          )}
        </div>
            ...
            >
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 'auto' }}>
                <button className="button" onClick={() => startTest(test)} style={{ width: '100%', padding: '10px' }}>
                  {test.completed ? '╨Я╤А╨╛╨╣╤В╨╕ ╤Б╨╜╨╛╨▓╨░' : '╨Э╨░╤З╨░╤В╤М'}
                </button>
